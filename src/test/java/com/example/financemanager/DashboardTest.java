...
public class DashboardTest {
    ...
    public void shouldHaveMenu(FxRobot robot) {
        ...
        robot.lookup(".menu-bar").queryAs(MenuBar.class).getMenus().forEach(menu -> {
            ...
            verifyThat(menu.getItems().get(1), MenuItemMatchers.hasText("Récap des dépenses"));
            // Test ajouté sur la nouvelle page
            verifyThat(menu.getItems().get(2), MenuItemMatchers.hasText("Récap des revenus"));
    ...
    public void shouldChangeStageWhenClickOnMenu(FxRobot robot) throws TimeoutException {
        ...
        robot.clickOn("Récap des dépenses", Motion.VERTICAL_FIRST, MouseButton.PRIMARY);
    ...
    // Test ajouté sur la nouvelle page
    @Test
    public void shouldChangeStageWhenClickOnMenuThenSalary(FxRobot robot) throws TimeoutException {
        robot.clickOn("Navigation");

        WaitForAsyncUtils.waitFor(2, TimeUnit.SECONDS, () -> robot.lookup("Revenus").match(NodeQueryUtils.isVisible()).tryQuery().isPresent());

        robot.clickOn("Récap des revenus", Motion.VERTICAL_FIRST, MouseButton.PRIMARY);
